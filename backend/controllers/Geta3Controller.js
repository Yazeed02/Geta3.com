...
exports.search_Geta3 = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const searchResults = await Geta3.find(query)
      .select("Title Description imgs brand carModel price")
      .limit(10)
      .exec();
    ...
    const updatedResults = searchResults.map(post => {
      ...
      return {
        ...post._doc,
        Cover: firstImageUrl,  // Or handle imgs array similar to other components
      };
    });
...
exports.editPost = asyncHandler(async (req, res) => {
  ...
  console.log('Received request to edit post:', postId); // Log the post ID
  ...
  try {
    ...
    // Log current post data before update
    ...
    post.Related_link = req.body.Related_link || post.Related_link;

    await post.save();

    console.log('Updated post data:', post); // Log the updated data
...
exports.Geta3_detail = asyncHandler(async (req, res) => {
  try {
    ...
    const userRating = req.user ? geta3.ratings.find((r) => r.user.toString() === req.user._id.toString()) : null;
...
exports.ratePost = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { rating } = req.body;

  if (rating < 1 || rating > 5) {
      return res.status(400).json({ message: 'Invalid rating value. It should be between 1 and 5.' });
  }

  try {
      const geta3 = await Geta3.findById(id);
      if (!geta3) {
          return res.status(404).json({ message: 'Post not found.' });
      }

      const existingRating = geta3.ratings.find((r) => r.user.toString() === req.user._id.toString());
      if (existingRating) {
          existingRating.rating = rating;
      } else {
          geta3.ratings.push({ user: req.user._id, rating });
      }

      geta3.averageRating = geta3.ratings.reduce((sum, r) => sum + r.rating, 0) / geta3.ratings.length;
      await geta3.save();

      res.status(200).json({ message: 'Rating updated successfully', averageRating: geta3.averageRating });
  } catch (error) {
      res.status(500).json({ message: 'Internal Server Error' });
  }
});
